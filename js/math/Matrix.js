...
export default class Matrix extends MathType {
  ...
  /**
   * @internal
   * Прямой доступ к приватной структуре БЕЗ копирования (только для внутреннего движка).
   * Вызывать аккуратно, не мутировать оригинальные данные!
   */
  get unsafeRows() {
    return this.#rows;
  }
  ...
  eq(other) {
    ...
    for (let i = 0; i < this.rowCount; i++) {
      ...
      for (let j = 0; j < this.colCount; j++) {
        if (rowA[j].not_eq(rowB[j]).value) return new BoolValue(false);
      }
    }
  ...
  max() {
    ...
    for (let i = 0; i < this.#rows.length; i++) {
      for (let j = 0; j < this.#rows[i].length; j++) {
        if (this.#rows[i][j].gte(_max).value) {
          _max = this.#rows[i][j];
        }
      }
    }
  ...
  min() {
    ...
    for (let i = 0; i < this.#rows.length; i++) {
      for (let j = 0; j < this.#rows[i].length; j++) {
        if (this.#rows[i][j].lte(_min).value) {
          _min = this.#rows[i][j];
        }
      }
    }
