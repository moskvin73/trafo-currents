...
export default class ASTNode {
  ...
  evaluate(context) {
    throw new Error("[ASTNode]: Метод [_internalEval]() не реализован.");
  }
...
export class NumberNode extends MathNode {
  ...
  [_internalEval](context) { return this.value; }
...
export class UnaryOpNodePlus extends UnaryOpNode {
  ...
  [_internalEval](context) { return this.argument.[_internalEval](context); }
}
...
export class UnaryOpNodeMinus extends UnaryOpNode {
  ...
  [_internalEval](context) { 
    const argVal = this.argument.[_internalEval](context);
...
export class AddNode extends BinaryOpNode {
  ...
  [_internalEval](context) {
    const { l, r } = dispatcher.promoteTypes(this.left.[_internalEval](context), this.right.[_internalEval](context));
    return l.add(r);
  }
...
export class AssignNode extends MathNode {
  ...
  [_internalEval](context) {
    const value = this.expression.[_internalEval](context);
    const sym = context.getSymbolById(this.id_name);
    sym.value = value;
...
export class ProgramNode {
  ...
  [_internalEval](context = {}) {
    ...
    for (const stmt of this.statements) {
      ...
      outputHTML += `<div>${stmt.[_internalEval](context)}</div>`;
    }
    return outputHTML;
  }
...
export class PrintNode extends ASTNode {
  ...
  [_internalEval](context) {
    return this.elements.map(element => {
      // 1. ОБРАБОТКА МАТЕМАТИЧЕСКИХ ВЫРАЖЕНИЙ
      if (element.type !== 'TEXT_BLOCK') {
        const evaluatedValue = element.[_internalEval](context);
...
export class CallNode extends MathNode {
  ...
  [_internalEval](context) {
    ...
    const evaluatedArgs = this.args.map(arg => arg.[_internalEval](context));
    const sym = context.getSymbolById(this.id_name);
