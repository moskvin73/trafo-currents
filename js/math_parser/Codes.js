...
class PushCommConstLoc extends PushCommConst {
    ...
    commandName() { return `push_loc [${this.loc}],`; }
...
class BinCommOpOp extends BaseBinComm {
    ...
    foldConstants(optimizedCode, simulatedStack, known_constants) {
        ...
        if (st_l.type === 'const' && st_r.type === 'const') {
            ...
            simulatedStack.push({ type: 'unknown' });
            optimizedCode.push({comm: this.recreateCommValueOp(new OpConst(st_l.value)), del: false });
        } else if (st_r.type === 'const') {
            // Пересоздать BinCommOpValue
            simulatedStack.push({ type: 'unknown' });
            optimizedCode.push({comm: this.recreateCommOpValue(new OpConst(st_r.value)), del: false });
            ...
            simulatedStack.push({ type: 'unknown' });
            optimizedCode[st_r.index].del = true;
            optimizedCode.push({comm: this.recreateCommOpValue(st_r.value), del: false });
        } else {
            simulatedStack.push({ type: 'unknown' });
            optimizedCode.push({ comm: this, del: false });
        }
...
class AssignCommValueConst extends Command {
    ...
    foldConstants(optimizedCode, simulatedStack, known_constants) {
        ...
        simulatedStack.push({type: 'const', value: value, index: idx});
        ...
        this.let_value.add_constant_value(known_constants, { value: value, index: idx });
        optimizedCode.push({ comm: this, del: false });     
    }
...
function createBinCode(operator, l_op, r_op, loc) {
    ...
    return processFn(l_op, r_op, loc);
}
    ...
    foldConstants(context) {
        ...
        for (const comm of this.#currentCode) {
            ...
            try { comm.foldConstants(optimizedCode, simulatedStack, known_constants); }
                ...
                errors = true;
                if (optimizedCode.length === c_len) optimizedCode.push({ comm: this, del: false });
        ...
        if (errors) return;
        const code = [];
        for (const item of optimizedCode) {
            if (!item.del) code.push(item.comm);
        }
