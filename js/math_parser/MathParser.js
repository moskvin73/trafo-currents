...
export class MathParser extends EventTarget {
  ...
  async #parseCode(code, signal) {
    ...
    while (this.c_token !== TokenType.EOF) {
      ...
      proc = Math.round(this.lexer.tokenEnd * 100 / rawCodeLen);
    }
  }
  ...
  #parseStatement(code, f_out = false) {
    ...
    switch(this.c_token)
    {
      ...
      case TokenType.RW_LET:
        this.#parseDeclarationStatement(code, f_out);
        break;
  ...
  #parsePlotInit() {
    ...
    return new PlotInitNode(id, diagram_id, mode, view_type, token_loc);
  }
      ...
      if (jumps.length > 0)
      {
        ...
        for (let i = 0; i < jumps.length; i++) {
          ...
          if (jump.node.len_code.type === MathParser.ALLOW_BREAK) {
            ...
            jump.node.len_code = -(jump.index + 2); // Условие + inc счётчика
          }
        }
      }
      ...
      if (jumps.length > 0)
      {
        ...
        for (let i = 0; i < jumps.length; i++) {
          ...
          if (jump.node.len_code.type === MathParser.ALLOW_BREAK) {
            ...
            jump.node.len_code = -(jump.index + (expCond ? 2 : 1));
          }
        }
      }
